...
If you really don't want to do that for some reason, consider imputing the missing values. The simplest available method replaces missing values in each column with the mean of the non-missing values in the respective column:
```{r}
m = mean(na.omit(x[,3]))
xi = x
xi[is.na(x)] = m
