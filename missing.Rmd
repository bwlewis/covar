...
`use=pairwise.complete.obs` and `use=complete` options in R's `cor`
and `cov` functions. The note illustrates potential issues with
a very simple pathological example.
...
assume bounded and valid numeric values which is actually pretty reasonable. But worse and weirder than that, R is inconsistent: `cor(x[,3],x[,3])` returns `NA` (!!!!)] choice in R's
`cov2cor` function, we see that `cor(x[,3],x[,3])=1`, but we don't know the
