...
want a method that works with general matrices $A$. We've also implicitly
assumed that we can efficiently compute a relatively low-rank truncated SVD of
potentially large  matrices required by step 1 of Algorithm \ref{prune}.
Fortunately, both needs are met by one algorithm.
...
We relax the column mean and scale assumptions without much additional
