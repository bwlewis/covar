...
We evaluted the algorithm using three publically-available gene expression
datasets.  The tests were performed on a desktop PC with a single AMD A10-7850K
3.7$\,$GHz Athlon quad-core CPU and 16$\,$GB 1333$\,$MHz unbuffered RAM running
the Ubuntu 12.04.5 GNU/Linux OS. Tests were performed with 64-bit R version
3.1.2 using double-precision arithmetic and the OpenBLAS library version
0.1alpha2.2-3 (based on Goto's BLAS version 1.13) with {\tt
OMP\_NUM\_THREADS=4}.  All examples use the reference R implementation
function, {\tt tcor()}, which can be installed for the R language from the
development GitHub repository using the {\tt devtools} package with:
...
from optimized BLAS routines, but not as much as it mostly makes use of Level 2
operations and Level 3 operations on sequences of smaller problems.
...
Anyone can quickly run the expample to experiment with the algorithm and
...
software\cite{biclust, bicat}. The data consist of 70 experiments invovling the
...
require(xtable)
ans = data.frame(Method=c("tcor","cor"), `Wall-clock time (s)`=c(t1, t2), `Peak excess memory use (MB)`=c(m1, m2), check.names=FALSE)
print(
    xtable(ans,
...
Performance benefits from the {\tt tcor()} function become apparent whith large
...
Broad GDAC dashboard\cite{gdac} of the Cancer Genome Atlas data. See the
supplemental code at XXX  (also embedded in this Sweave document) for examples
of efficiently converting the downloaded sample expression data into an R
matrix.

Eight columns with constant zero values were removed from the data in a
pre-processing step, leaving an $878\times 20524$ matrix. The example computes
...
#h = 2      # header lines
# Read the first header line with sampe IDs:
...
set of  3,572 pairs of vectors metting the correlation threshold of 0.99, but
...
XXX Two basic extensions, include functions/options for this in the package.

The main ideas in Algorithm \ref{prune}, although applied to correlation,
easily extend to any distance metric.


Algorithm \ref{prune} can be adapted to find pairs of highly anti-correlated
vectors by changing the distance metric...XXX
...
\bibitem{gdac}{\url{http://gdac.broadinstitute.org/runs/stddata__2015_08_21/data/BRCA/20150821/gdac.broadinstitute.org_BRCA.Merge_rnaseq__illuminahiseq_rnaseq__unc_edu__Level_3__gene_expression__data.Level_3.2015082100.0.0.tar.gz}}
...
\bibitem{tcor}
Lewis, Bryan,  \url{https://github.com/bwlewis/covar/blob/master/threshold/tcor.R}.
