...
For later convenience, we write $s=\mathrm{diag}(\Sigma)$ to be the
diagonal vector of the matrix $\Sigma$.
...
\begin{enumerate}
\item Compute the truncated SVD $AV_{1:p} = U_{1:p}s_{1:p}$, $V_{1:p} = [v_1, v_2, \ldots, v_p]$.
...
e_{\tilde{j}}^T(D^{\tilde{i}} P V_{1:p} )^2 s_{1:p} \le 2(1-t)
\right\},
\]
