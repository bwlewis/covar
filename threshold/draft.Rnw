...
of the most highly-correlated paris using dimensionality-reduction techniques
...
instance, the minimum distance between the points shown in the example in
...
\begin{equation}
\label{eqasymdist1}
|| X_i - X_j ||^2 \sim R + \frac{\Delta_{ij} + 1}{2} S + 
  \frac{\Delta_{ij} - 1}{2} T
\end{equation}
...
Then
\begin{equation*}
||a_i - a_j||^2 \xrightarrow[m]{D} \mathcal{N} \left(2 + \Delta_{ij},  
  \frac{5 + \Delta_{ij}^2}{m} \right) 
\end{equation*}
\end{lemma}
