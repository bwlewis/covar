...
the thresholded correlation problem; see, for example\cite{zf,mnl} and the
...
Let $x\in\mathcal{R}^n$ represent the vector of column means of the matrix $A$,
$W\in\mathcal{R}^{n\times n}$ be a diagonal matrix with the reciprocal of the
column norms of $A$ along the diagonal, and let $e\in\mathcal{R}^m$ be a vector
of all ones. Then $(A - ex^T)W$ is a centered matrix with zero column means,
and scaled to have unit column norms, and

XXX

IRLBA, based on the Lanczos process, is an iterative method based on
matrix-vector products involving the matrix $A$.
