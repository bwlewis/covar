...
\cite{xstk1},\cite{xstk2},\cite{xbm} and
...
example\cite{mnl,xbm,xstk1,xstk2,zf} and the references therein.  The approach
described here prunes along an intuitive path of decreasing variance defined by
the data and computed by the SVD.
