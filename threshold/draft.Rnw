...
later convenience, we write $s=[\sigma_1, \sigma_2, \ldots, \sigma_p]^T$ to be
...
\begin{enumerate}
\item Compute the truncated SVD $AV_{1:p} = U_{1:p}\mathrm{diag}(s_{1:p})$, $V_{1:p} = [v_1, v_2, \ldots, v_p]$.
