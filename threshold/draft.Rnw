...
%%
%% FIRST NOTE, you need the 'xtable' and 'biclust' packages installedi in R.
...
available for \proglang{R} \citep{irlbar}, \proglang{Matlab} \citep{irlbam},
and \proglang{Python} \citep{irlbap}.
...
The \proglang{R} implementation \citep{irlbar} of IRLBA includes arguments for centering and
...
datasets from the \proglang{R} {\tt biclust} \citep{biclust} package and from
...
The native \proglang{R} {\tt cor()} function is a general function that can compute
...
EisenYeast} data from the {\tt biclust} \proglang{R} package \citep{biclust}.
...
Affymetrix probes, represented as a $80\times 6221$ real-valued matrix.  The
example finds all pairs of columns (gene expression measurements) that meet or exceed a
Pearson correlation value of 0.95.  We use a projected dimension $p=10$
in {\tt tcor()}.
\begin{small}
<<eval=TRUE,echo=TRUE>>=
library(tcor)
...
print(
    xtable(ans,
        caption = "EisenYeast results, threshold=0.95", label = "EisenYeast", caption.placement = "top")
...
See the \proglang{R} package implementation \citep{sup} for examples of
...
computing thresholded correlation among sets of large numbers of vectors,
...
Our simple brute force implementation was not possible due to memory constraints.
We remark that the matrix multiplication central to the brute force
implementation can be decomposed into a sequence of smaller steps to
fit within given memory constraints. However, the brute force algorithm would still
need to evaluate all of the nearly 160 billion possible vector pair correlation
values.
...
{\tt tcor()} implementation uses \proglang{R}'s {\tt foreach} \citep{foreach} framework to
run on arbitrary parallel configurations (``back ends''). We simply registered
a {\tt doRedis} \citep{doredis} parallel back end with 16 \proglang{R} workers per computer
...
@
From a set of almost 160 billion pairs of vectors, Algorithm \ref{prune} finds 913,601
...
subspaces applies to distance metrics similarly to correlation. Lemma
\ref{sumlemma} decomposes Euclidean distance between two vectors in terms of
singular basis vectors and Algorithm \ref{prune} can be applied directly to the
problem of finding all pairs of vectors that meet a thresholded Euclidean
...
reference \proglang{R} implementation \citep{sup} includes functions for thresholded
...
We present a simple algorithm and reference \proglang{R} implementation based on a
truncated singular value decomposition (SVD) that efficiently computes
thresholded subsets of large correlation and distance matrices.  We show that
the natural connection between data covariance and the SVD can result in
substantial computational and memory savings, and illustrate these savings with
a few genomics data experiments.  Moreover, the main computational work in the
algorithm is easily parallelized and easily deployed across computational
clusters.
