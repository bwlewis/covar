...
adjacent vectors fall below the a correlation threshold of $0.95$ in the
...
development version of IRLBA GitHub repository using the {\tt devtools} package
with:
...
\subsection{BicatYeast small example}

The first example is a tiny proof of concept that provides the data for Figure
\ref{fig1} and illustrates our timing and memory-use measurement methodology.
...
# Compare results!
cat("brute force vector pairs found", nrow(pairs)/2, "\n")
cat("tcor vector pairs found       ", nrow(tx[[1]]), "\n")
@
\end{small}
...
115 vector pairs that exceed the specified correlation threshold of 0.95, with
comparative timing and memory use shown in the following table.

\begin{center}
\begin{tabular}{l|rr}
 & {\tt tcor()} & native {\tt cor()} \\
Wall-clock time (s) & 0.26 & 0.02 \\
Excess peak memory use (MB) & 29.2 & 26.2 \\
\end{tabular}
\end{center}
...
Brute-force computation benefits substantially from sustained use of BLAS Level
3 operations (matrix multiplication), achieving high utilization of available
CPU floating point capacity. The tcor method also benefits from optimized BLAS
routines, but not as much as it mostly makes use of Level 2 operations and
Level 3 operations on sequences of smaller problems.
...
%print(
%    xtable(head(iris),
%        caption = "blah", label = "tab:five", table.placement = "!h", caption.placement = "top")
