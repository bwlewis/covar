...
XXX add discussion of applications (especially genomics?) and previous work,
...
and that each column has unit norm $\|a_j\|^2 = 1$. Here and below, $\|\cdot\|$
...
correlation and covariance matrices are the same.  Section \ref{examples} below
...
diagonal entries $\sigma_1 \ge \sigma_2 \ge \cdots \ge \sigma_k > 0$.  Denote
the columns of $V$ as $V=[v_1, v_2, \ldots, v_k]$, where each
...
outside the correlation threshold.
\end{lemma}
...
that paris of indices corresponding to correlated vectors can exhibit.
...
s=svd(A)
i=order(s$v[,1])
plot(s$v[i,1], ylab=expression(Pi * v[1]), xlab="Sorted index", pch='.')
t=0.99
...
difference matrix with diagonal entries $-1$, $i$th super-diagonal entries $1$,
and zeros elsewhere.  Consider just adjacent points of $P v_1$.  Then $D^1P
v_1$ consists of the differences of adjacent entries in $P v_1$.  Entries
such that $\sigma_1^2 (D^1 P v_1)^2 > 2(1-t)$, where the exponentiation is
applied element-wise, correspond to pairs of vectors that do not meet
the correlation threshold. This is unlikely to rule out many pairs of vectors
in most problems. For example, the minimum distance between the points shown in
...
pairs of vectors outside the correlation threshold. For example, including
only one more term finds that only 116 out of 418 pairs of adjacent vectors
in the example shown in Figure \ref{fig1} satisfy
\[
\sigma_1^2 (D^1 P v_1)^2 +
\sigma_2^2 (D^1 P v_2)^2 \le 2(1-t),
\]
...
throughout. Including five terms reduces the total to only 11 pairs of vectors.
In general, using $p\le k$ terms boils down to evaluating the expression
\begin{equation}\label{proj}
(D^1 P V_{1:p} )^2 \mathrm{diag}(\Sigma_{1:p}) \le 2(1-t),
\end{equation}
where 
$V_{1:p}$ and $\Sigma_{1:p}$ denote the first $p$ columns of the
matrices $V$ and $\Sigma$, respectively,
$(D^1PV_{1:p})^2$ denotes element-wise exponentiation, 
and $\mathrm{diag}(\Sigma)$ denotes the diagonal of the matrix $\Sigma$.

Pairs of vectors beyond the correlation threshold that lie more than one index
...
subspace dimension $p<<n$.
\begin{enumerate}
\item Compute the truncated SVD $AV_{1:p} = U_{1:p}\Sigma_{1:p}$, $V_{1:p} = [v_1, v_2, \ldots, v_p]$.
...
\item Compute a set of candidate pairs $(\tilde{j},\tilde{j}+\tilde{i})$ with respect to the index order defined by the permutation $P$
\[
\bigcup_{\tilde{i}=1}^\ell
\left\{
(\tilde{j},\tilde{j}+\tilde{i}) : 
e_{\tilde{j}}^T(D^{\tilde{i}} P V_{1:p} )^2 \mathrm{diag}(\Sigma_{1:p}) \le 2(1-t)
\right\},
\]
...
of vectors outside the correlation threshold with truncated singular value
decompositions of only relatively low dimension $p$.  The choice of $p$ is a
...
\section{Efficient truncated SVD for general matrices}

We have so far assumed that the mean of each column vector of $A$ is zero and
the columns are scaled to have norm 1, but we really want a method that works
with general matrices $A$. We've also implicitly assumed that we can
efficiently compute a relatively low-dimensional truncated SVD of potentially
large data matrices required by step 1 of Algorithm \ref{prune}.  Fortunately,
both needs are met by one algorithm.

The augmented, implicitly restarted Lanczos bidiagonalization (IRLBA) method of
Baglama and Reichel \cite{br} efficiently computes truncated singular value
...
The R implementation includes efficient methods for restarting the algorithm
to increase the subspace dimension (see step 5 of Algorithm \ref{prune}), and
efficient methods for computing the SVD of scaled and centered matrices.
