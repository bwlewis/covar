...
learning applications\cite{ml1, ml2} and are also important to time series
and image processing applications\cite{timeseries, svd-similarity}.
...
described for the thresholded correlation problem.  For example the TAPER and
related algorithms of Xiong et al. base pruning rules on the frequency of
mutually co-ocurring observations (rows) among vector pairs; see \cite{prune1,
prune2} and the references therein. Related methods only approximate a set
containing the most highly-correlated pairs using hashing
...
like the discrete Fourier transform\cite{timeseries}. The method of Wu, et
al.\cite{svd-similarity}, conceptually very similar to our approach, uses the
singular value decomposition (SVD) to find all pairs of close vectors with
respect to a distance metric (instead of correlation).  Our method finds all of
...
correlation and covariance matrices are the same.  Section \ref{irlba}
...
Lemma \ref{distlemma} equates the problem of finding pairs of highly
correlated column vectors from the matrix $A$ with a problem of finding pairs
...
\ref{vdist} that for any $i,j=1,2,\ldots,n$, if $\sigma_1^2 |e_i^Tv_1 -
e_j^Tv_1|^2 > 2(1-t)$ then $\mathrm{cor}(a_i,a_j) < t$.  Let $\ell$ be the
...
index difference that pairs of indices corresponding to correlated vectors
...
Figure \ref{fig1} is less than $\sqrt{2(1-t)}/\sigma_1$, and no pruning occurs.
...
\right\}.
\]
\item If there are judged to be  ``too many'' candidate pairs, increase $p$ and
restart IRLBA to compute 
$AV_{1:p} = U_{1:p}\mathrm{diag}(s_{1:p})$ and go to step 4.
...
additional computational or storage overhead as follows.  Let
