...
of gene expression values may be used in the discovery networks of genes
...
learning applications\cite{ml1, ml2}. The problem is also important to time
series and image processing applications, see for example\cite{timeseries,
svd-similarity}.
...
the thresholded correlation problem; see, for
example\cite{prune1, prune2, prune3}
